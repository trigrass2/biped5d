...
class T100(Canopen_control_init):
    """
    ...
    def sent_position(self,position,velocity = 0.03):
        ...
        if(self.mode == 1):
            ...
            self.node.sdo[0x6081].raw = 10 * self._T100_msg_to_device(velocity)  # 0.02 rad/s
            self.node.sdo[0x6060].raw = 1  # profile position mode
            self.node.sdo[0x6086].raw = 0
            self.node.sdo.download(0x607a, 0x0, self._decTohex_32(self._T100_msg_to_device(position)))
    ...
    def sent_velocity(self,velocity):
        ...
        if self.mode == 3:  # Profiled Velocity
            ...
            velocity = 10 * self._T100_msg_to_device(velocity)
    ...
    def get_position(self):
        ...
        :return position(rad)
        """
        return self._T100_msg_from_device(self.node.sdo[0x6064].phys)  # rad
    ...
    def get_velocity(self):
        ...
        """
        return (self._T100_msg_from_device(self.node.sdo[0x606c].phys)) / 10 # rad/s
    ...
    def _T100_msg_to_device(self, position):
        """
        rad -> mdeg * resolution of encoder * reduction ratio / 360 degree
        :return:
        """
        position = degrees(position) * 4096 * 480 / 360
        return int(position)

    def _T100_msg_from_device(self, position):
        """
        actual position [count] -> rad
        :return:
        """
        position = radians(position) * 360 / 480 / 4096
        return position
